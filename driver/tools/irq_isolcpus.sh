...
all_mask=`echo "obase=16; (2 ^ $all_cpus) - 1" | bc | tr A-F a-f`

# Find mask of available CPUs by looking at init's mask
init_pid=`pidof init`
init_mask=`taskset -p $init_pid | awk -F':' '{print $2}' | tr -d ' '`
...
irqs=`grep "$DRIVER" /proc/interrupts | awk -F':' '{print $1}' | tr -d ' '`
if [ "x$irqs" = "x" ]; then
...
for file in /proc/irq/[0-9]*/smp_affinity; do
	echo $init_mask > $file 2>/dev/null
done
